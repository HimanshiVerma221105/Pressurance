...
loader.load('./../models/humanModel.fbx', (fbx) => {
  ...
  fbx.position.set(-3.4, -10, 0);
  scene.add(fbx);
  // Create all head pain zones
  createPainZoneBox(1.3, 0.6, 1, 'Elbow_Pain', -0.4, 0.15, -0.2);
  createPainZoneBox(1.21, 0.4, 1, 'Hand_Pain', 1.8, 0.15, 0.3);
});
...
function onClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (currentlyGlowing === clicked) {
      ...
      // Log to server
      fetch('http://localhost:3000/log-click', {
        method: 'POST',
        headers: {
